/* File: /assets/js/pages/valoraciones.js */
(function () {
    'use strict';

    // Evitar doble inicialización si el bloque se inserta más de una vez
    if (window.__huichplyValoracionesMounted) return;
    window.__huichplyValoracionesMounted = true;

    const API_ENABLED = (window.__HUICHPLY_API_ENABLED__ !== false);

    // Por defecto: backend real vive en /api
    // Si quieres cambiarlo, define window.__HUICHPLY_API_BASE__ = "https://tu-dominio.com/api"
    const API_BASE = String(window.__HUICHPLY_API_BASE__ ?? window.__REVIEWS_API_BASE__ ?? '/api')
        .replace(/\/+$/, '');
    ...
    async function renderizar() {
        ...
        // 1) Intentar API real si existe; si falla, fallback local
        if (API_ENABLED) {
            try {
                const featured = await apiGet(`/reviews/featured?limit=${PUBLISH.limitCards}`);
                const summary = await apiGet('/reviews/summary');
